...
public class TyrvingItem extends LodestoneSwordItem implements IMalumEventResponderItem {
    ...
    public void hurtEvent(LivingHurtEvent event, LivingEntity attacker, LivingEntity target, ItemStack stack) {
        ...
        if (attacker.level instanceof ServerLevel) {
            ...
            MALUM_CHANNEL.send(PacketDistributor.TRACKING_ENTITY.with(() -> target), new MajorEntityEffectParticlePacket(SpiritTypeRegistry.ELDRITCH.get().getPrimaryColor(), target.getX(), target.getY() + target.getBbHeight() / 2, target.getZ()));
        }
    }
